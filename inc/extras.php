 ...
 * @return array
 */
function clickmaterial_page_menu_args($args)
{
    $args['show_home'] = true;
    return $args;
}

add_filter('wp_page_menu_args', 'clickmaterial_page_menu_args');
 ...
 * @return array
 */
function clickmaterial_body_classes($classes)
{
...
add_filter('body_class', 'clickmaterial_body_classes');
 ...
 */
function clickmaterial_wp_title($title, $sep)
{
    if (is_feed()) {
        return $title;
    }
    ...
    if (($paged >= 2 || $page >= 2) && !is_404()) {
        $title .= " $sep " . sprintf(__('Page %s', 'clickmaterial'), max($paged, $page));
    }

    return $title;
}

add_filter('wp_title', 'clickmaterial_wp_title', 10, 2);
 ...
 * @return void
 */
function clickmaterial_setup_author()
{
    global $wp_query;
...
add_action('wp', 'clickmaterial_setup_author');
...
function modify_read_more_link()
{
    ...
    return '<p><a class="read-more" href="' . get_permalink() . '">' . __('Read More', 'clickmaterial') . '</a></p>';
}
